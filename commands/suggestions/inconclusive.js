...
module.exports = {
    name: 'inconclusive',
    description: 'Set the status of any suggestion as inconclusive',
    args: true,
    ...
    async execute(msg, args) { // TODO: Error checking
        ...
        const channel = msg.guild.channels.cache.get(serverData['suggestions']['inconclusivechannel']);
        if(!channel) return utils.sendEmbeddedResponse(msg, false, true, `I can't find the channel that inconclusive suggestions should get sent to. Please make sure I have an available channel to send inconclusive suggestions to.`, null, 0xFF0000); // If the channel cannot be found...
        ...
        channel.send({embed: embed}).then(message => { // Then resend it to the new channel
            ...
            if(serverData['suggestions']['dm']) {
                ...
                if(getMember) {
                    embed['title'] = "Your suggestion was marked as inconclusive.";
