...
module.exports = {
    name: 'potential',
    description: 'Potential any suggestion.',
    args: true,
    ...
    async execute(msg, args) { // TODO: Error checking
        ...
        const channel = msg.guild.channels.cache.get(serverData['suggestions']['potentialchannel']);
        if(!channel) return utils.sendEmbeddedResponse(msg, false, true, `I can't find the channel that potential suggestions should get sent to. Please make sure I have an available channel to send potential suggestions to.`, null, 0xFF0000); // If the channel cannot be found...
        ...
        channel.send({embed: embed}).then(message => { // Then resend it to the new channel
            ...
            if(serverData['suggestions']['dm']) {
                ...
                if(getMember) {
                    embed['title'] = "Your suggestion was marked as potential.";
